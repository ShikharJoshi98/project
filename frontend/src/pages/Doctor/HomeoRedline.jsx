...
const HomeoRedline = () => {
  ...
  useEffect(() => {
    ...
    const filteredRedline = HomeoRedline.filter((item) =>item.name.toLowerCase().includes(searchTerm)||item.description.toLowerCase().includes(searchTerm));
  ...
  const generateTablePDF = () => {
        ...
        doc.setFontSize(18);
        doc.text("Redline Symptoms Descriptions", 14, 40);
      
        // Define table columns
        const tableColumn = ["Serial No.", "Disease", "Description"];
        const tableRows = HomeoRedline.map((redline, index) => [
          index + 1,
          redline.name,
          redline.description,
        ]);
      ...
      const generatePDF = (redline) => {
        ...
        doc.setFontSize(18);
        doc.text("Redline Symptoms Description", 14, 40);
      
        const tableColumn = ["Serial No.", "Disease", "Description"];
        const tableRows = [[1, redline.name, redline.description]];
  ...
  async function deleteCol(id) {
        try {
          ...
          const updatedRedlines = HomeoRedline.filter((redline) => redline._id !== id);
        docStore.setState((prevState) => ({
          ...prevState,
          Homeo: updatedRedlines, 
        }));
        } catch (error) {
        ...
        <DocSidebar/>
          ...
          <h1 className=' text-blue-500 font-semibold mb-3 text-lg md:text-2xl mt-4'>{currentDate}</h1>
                    ...
                    <button  onClick={() => handleSectionChange("medicine", "/homeo-book-medicine")} className={`cursor-pointer border-1 hover:scale-102 transition-all duration-300 bg-blue-300  p-2 hover:bg-blue-600 hover:text-white rounded-lg`}>MEDICINE NAME</button>
                        <button oonClick={() => handleSectionChange("disease", "/homeo-book-disease")}   className={`cursor-pointer border-1 hover:scale-102 transition-all duration-300 bg-blue-300  p-2 hover:bg-blue-600 hover:text-white rounded-lg`}>DISEASE NAME</button>
                        <button onClick={() => handleSectionChange("redline", "/homeo-book-redline")}   className={`cursor-pointer border-1 hover:scale-102 transition-all duration-300 bg-blue-500 text-white  p-2 hover:bg-blue-600 hover:text-white rounded-lg`}>RED LINE SYMPTOMS</button>
                    </div>
                    <div>
          ...
          <div>
            ...
            <Input onChange={(e) => setSearchTerm(e.target.value)} icon={Search} type="text" name="name" placeholder='Enter Symptom or description' />
    ...
    <table className="min-w-full bg-white border border-gray-300 shadow-md rounded-lg">
            <thead className=''>
              <tr className=" bg-blue-500 text-white text-lg">
                ...
                <th className="py-2 px-4 border">Redline Symptoms</th>
                ...
                <th className="py-2 px-4 border">PDF</th>
                    ...
                    <tbody>
      {filteredRedline.map((redline, idx) => (
        <tr key={idx} className="bg-blue-100">
          ...
          <td className="py-3 px-4 border text-center">
            {editingRow === idx ? (
              ...
              <button onClick={() => handleEditClick(idx, redline)} className="  p-1 rounded">
                <SquarePen/>
              </button>
            )}
          </td>
          <td onClick={()=>deleteCol(redline?._id)} className="py-3 px-4 border text-center cursor-pointer border-black text-red-500">{ <Trash />}</td>
          <td onClick={() => generatePDF(redline)} className="py-3 px-4 border text-center cursor-pointer">{ <FaRegFilePdf size={25} />}</td>
        </tr>
      ))}
    </tbody>
