...
const OrderPaymentModal = ({ setSubmit, type, onClose, orderId }) => {
    const { getOrderPaymentDetails, getMedicalOrderPaymentDetails, orderPaymentDetails } = docStore();
    ...
    useEffect(() => {
        if (orderId && type === 'item') {
            getOrderPaymentDetails(orderId);
        }
        else if (orderId && type === 'medicine') {
            getMedicalOrderPaymentDetails(orderId);
        }
    }, [getOrderPaymentDetails, getMedicalOrderPaymentDetails, orderId]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (type === 'item') {
                const response = await axios.post(`${DOC_API_URL}/addPaymentDetails/${orderId}`, formValues);
            }
            else if (type === 'medicine') {
                const response = await axios.post(`${DOC_API_URL}/addMedicalPaymentDetails/${orderId}`, formValues);
            }
            setSubmit(prev => !prev);
            setFormValues({
                transactionDetails: '',
            ...
            <div className="bg-[#e9ecef] max-h-[90vh] max-w-[99vw] overflow-y-auto flex flex-col w-full  rounded-xl p-6 md:p-10 shadow-lg">
                ...
                {orderPaymentDetails && orderPaymentDetails?.details_added_flag === true ? <div className="mx-auto">
                    ...
                    <form onSubmit={handleSubmit} className='flex flex-col gap-3'>
                        ...
                        <div className='flex flex-col gap-2'>
                            <h1>Bill Number</h1>
                            <Input icon={RiBillFill} onChange={handleInputChange} value={formValues.billNo} name="billNo" placeholder='Add Bill No.' />
                        </div>
                        <div className='flex flex-col gap-2'>
                            <h1>Bill Amount</h1>
                        ...
                        <button className='mx-auto bg-blue-500 text-white py-1 px-3 rounded-md text-lg mt-6'>Add</button>
                    </form>}
            </div>
        </div>
    )
