...
const OrderItemHistoryModal = ({ location, onClose }) => {
    ...
    const filteredOrders = ordersPlaced.filter(order => {
        ...
        const matchesVendor = order.formRows.some(row => {
            const vendors = Array.isArray(row.vendor) ? row.vendor : [row.vendor];
            return vendors.some(v => v?.toLowerCase().includes(search));
        });

        const matchesBillNumber = order.billNumber
            ?.toString()
            .toLowerCase()
            .includes(search);

        return matchesVendor || matchesBillNumber;
    });
    const addBillId = (id) => {
        setChecked((prev) =>
            prev.includes(id)
    ...
    return (
        <div className="bg-black/50 z-60 fixed inset-0 flex items-center justify-center p-4">
            <div className="bg-[#e9ecef] max-h-[90vh] max-w-[99vw] overflow-y-auto flex flex-col w-full  rounded-xl p-6 md:p-10 shadow-lg">
                ...
                <Input icon={CiSearch} onChange={(e) => setSearchTerm(e.target.value)} placeholder='Search by Vendor or Bill Number' />
                    ...
                    <table className="min-w-full border border-gray-300 bg-white shadow-md ">
                        <thead className="bg-[#337ab7]  text-white">
                            <tr >
                                ...
                                <th className="px-2 py-4 ">Payment Status</th>
                            </tr>
                        </thead>
                        <tbody>
