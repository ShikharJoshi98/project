...
const HRMedicine = () => {
    ...
    const medicineArray = appointments.filter((appointment) => (appointment?.date === currentDate && appointment?.complete_appointment_flag === true && appointment?.appointmentType === medSection && appointment?.branch === user?.branch) && (appointment?.PatientCase?.fullname?.toLowerCase().includes(searchTerm.toLowerCase()) || appointment?.PatientCase?.casePaperNo?.toLowerCase().includes(searchTerm.toLowerCase()) || appointment?.PatientCase?.phone?.toLowerCase().includes(searchTerm.toLowerCase())));
            ...
            <div className="flex">
                ...
                <div className='bg-opacity-50 backdrop-filter backdrop-blur-xl bg-gradient-to-br from-blue-300 via-blue-400 to-sky-700  min-h-screen  w-full overflow-hidden '>
                    <div className='bg-[#e9ecef]  w-auto p-5 mx-10 my-6 rounded-lg '>
                        <h1 className='p-4 text-center font-semibold text-[#337ab7] text-xl sm:text-3xl md:text-5xl'>{medSection.toUpperCase()} MEDICINE {user?.branch.toUpperCase()}</h1>
