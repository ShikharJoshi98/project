...
const RegenerateCertificateModal = ({ setSubmit, onClose, certificate }) => {
    ...
    const [isLetterHead, setLetterHead] = useState('');
    ...
    useEffect(() => {
        setLetterHead(certificate?.templateImage);
    }, []);
        ...
        <div className="bg-black/50 z-60 fixed inset-0 flex items-center justify-center p-4">
            <div className="bg-[#e9ecef] max-h-[99vh] max-w-[99vw] overflow-y-auto flex flex-col w-full rounded-xl p-6 md:p-10 shadow-lg">
                ...
                <div className='sm:flex grid grid-cols-2 mt-10 sm:flex-row text-white font-semibold  gap-2 sm:gap-9 justify-center items-center md:gap-9 text-[6px] sm:text-[8px] md:text-sm'>
                    <button onClick={() => { generateMedicalCertificate(isLetterHead, formValues, certificate?.patient, certificate?.doctor); }} className='cursor-pointer hover:scale-102 transition-all duration-300 bg-blue-500 p-2 hover:bg-blue-600 rounded-lg'>MEDICAL CERTIFICATE</button>
                    <button onClick={() => { generateFitnessCertificate(isLetterHead, formValues, certificate?.patient, certificate?.doctor); }} className='cursor-pointer hover:scale-102 transition-all duration-300 bg-blue-500 p-2 hover:bg-blue-600 rounded-lg'>FITNESS CERTIFICATE</button>
                    <button onClick={() => { generateUnfitCertificate(isLetterHead, formValues, certificate?.patient, certificate?.doctor); }} className='cursor-pointer hover:scale-102 transition-all duration-300 bg-blue-500 p-2 hover:bg-blue-600 rounded-lg'>UNFIT CERTIFICATE</button>
                    <button onClick={() => { generateTravellingCertificate(isLetterHead, formValues, certificate?.patient, certificate?.doctor); }} className='cursor-pointer hover:scale-102 transition-all duration-300 bg-blue-500 p-2 hover:bg-blue-600 rounded-lg'>TRAVELLING CERTIFICATE</button>
                </div>
