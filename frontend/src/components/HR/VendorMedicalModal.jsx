...
const VendorModal = ({ onClose }) => {
  ...
  const { getMedicalVendors, vendors, editMedicalVendor, addMedicalVendor } = useStore();
  const cols = ['vendorname', 'contact', 'email', 'address'];
  async function handleSave(editedData) {
    try {
      await editMedicalVendor(editedData._id, editedData.vendorname, editedData.contact, editedData.email, editedData.address);
      setEditingRow(null);
  ...
  return (
    <div className="bg-black/50 z-60 fixed inset-0 flex items-center justify-center p-4">
      <div className="bg-[#e9ecef] max-h-[90vh] max-w-[80vw] overflow-y-auto   flex flex-col w-full  rounded-xl p-6 md:p-10 shadow-lg">
        <button onClick={onClose} className="place-self-end cursor-pointer transition-all duration-300 hover:text-white hover:bg-red-500 rounded-md p-1"><RxCross1 size={24} /></button>
        <h1 className="text-blue-500 text-2xl md:text-3xl mb-6 text-center font-semibold">Vendors</h1>
          ...
          <div className="bg-white p-5 rounded-lg max-w-[834px]">
            ...
            <form onSubmit={handleSubmit} >
              <div className='mb-3'>
                ...
                <Input onChange={(e) => setname(e.target.value)} icon={CiUser} value={vendorname} type="text" placeholder="Enter Vendor Name" />
              </div>
              <div className='mb-3'>
                  ...
                  <tr key={idx} className="hover:bg-blue-300 bg-blue-200 transition-all">
                    ...
                    {cols.map((col) => (
                      <td key={col} className="border border-gray-300 px-4 py-2">
                        {editingRow === idx ? (
                          <input type="text" value={editedData[col] || ""} onChange={(e) => handleChange(e, col)} className="border border-gray-400 p-1 w-full"/>
                        ) : (
                          vendor[col]
                        )}
                      </td>
                    ...
                    <td className="border border-gray-300 px-4 py-2 text-center">
                      {editingRow === idx ? (
                        <button className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-700 transition-all" onClick={() => handleSave(editedData)}>Save</button>
                      ) : (
                        <button className="bg-blue-500 cursor-pointer hover:scale-102 text-white px-4 py-2 rounded hover:bg-blue-700 transition-all" onClick={() => handleEditClick(idx, vendor)}>Update</button>
                      )}
                    </td>
                  </tr>
                ))}
