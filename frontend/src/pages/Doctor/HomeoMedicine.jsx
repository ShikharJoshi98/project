        ...
        <div className='bg-opacity-50 backdrop-filter backdrop-blur-xl bg-gradient-to-br from-blue-300 via-blue-400 to-sky-700  min-h-screen  w-full overflow-hidden '>
          ...
          <div className='bg-[#e9ecef]  w-auto p-5 mx-10 my-6 rounded-lg '>
            ...
            <div className='sm:flex grid grid-cols-2 mt-5 sm:flex-row text-stone-800 font-semibold  gap-2 sm:gap-9 justify-center items-center md:gap-9 text-[10px] sm:text-base md:text-lg'>
              <button onClick={() => handleSectionChange("medicine", "/homeo-book-medicine")} className={`cursor-pointer border-1 hover:scale-102 transition-all duration-300 bg-blue-500 text-white  p-2 hover:bg-blue-600 hover:text-white rounded-lg`}>MEDICINE NAME</button>
              <button onClick={() => handleSectionChange("disease", "/homeo-book-disease")} className={`cursor-pointer border-1 hover:scale-102 transition-all duration-300 bg-blue-300  p-2 hover:bg-blue-600 hover:text-white rounded-lg`}>DISEASE NAME</button>
              ...
              <form onSubmit={handleSubmit} className="mx-auto relative z-10 my-8 bg-white/80 h-auto p-8 min-w-full border rounded-xl text-zinc-600 text-sm shadow-lg">
                <div className="flex flex-col gap-4 m-auto">
                  <div className="flex flex-col gap-2">
                    <h1 className='text-black '>Medicine Name</h1>
                    <Input icon={AiFillMedicineBox} type="text" name="name" value={formValues.name} onChange={handleInputChange} placeholder='Enter Medicine Name' required />
                  </div>
                </div>
                <div className='mt-3 '>
                    ...
                    <tr key={idx} className="bg-blue-100">
                      ...
                      <td className="py-3 px-4 border text-center">
                        {editingRow === idx ? (
                          ...
                          <button onClick={() => handleEditClick(idx, medicine)} className="p-1 rounded">
                            <SquarePen />
                          </button>
                        )}
                      </td>
