...
const TodayCollection = () => {
    ...
    const { getCollection, collection, getDetails, employees, getEveryCollection } = useStore();
    const [employee, setEmployee] = useState("");
    const [isShift, changeShift] = useState('');
    ...
    useEffect(() => {
        const fetchCollection = async () => {
            try {
                setLoading(true);
                if (isShift === 'All') {
                    await getEveryCollection(location.location);
                }
                else {
                    await getCollection(location.location, isShift);
                }
            } catch (error) {
    ...
    return (
        <div className='bg-gradient-to-br from-blue-300 via-blue-400 to-sky-700 overflow-hidden min-h-screen w-full p-8'>
            <div className='bg-[#e9ecef] w-auto p-5 mx-10 my-6 rounded-lg'>
                ...
                {clinic[0]?.shifts?.length > 0 &&
                    <div className='h-12 bg-[#c8c8ce] w-fit mx-auto my-6 rounded-[18px]'>
                        {clinic?.filter((clinic, _) => clinic?.branch === location.location).map((detail, index) => (
                            detail?.shifts?.map((shift, shiftIndex) => (
                                <button key={shiftIndex - index} onClick={() => changeShift(shift)} className={`${isShift === shift ? 'bg-blue-700 rounded-[18px] text-white' : ''} py-2.5 px-5 text-lg cursor-pointer`}>{shift}</button>
                            ))
                        ))}

                        <button onClick={() => changeShift('All')} className={`py-2.5 px-5 ${isShift === 'All' ? 'bg-blue-700 rounded-[18px] text-white' : ''} text-lg cursor-pointer`}>All</button>
                    </div>}
                ...
                {collectionType === 'Collections' && <div>
                    ...
                    <div className='flex flex-col gap-2'>
                        ...
                        <div className='relative'>
                            ...
                            <select defaultValue="" onChange={(e) => setEmployee(e.target.value)} name="patient" required id="patient" className='py-2 pl-9 bg-white rounded-lg border border-gray-400 w-full focus:outline-none focus:ring-2 focus:ring-blue-300 text-zinc-900'>
                                <option value="" disabled className='font-normal' >Select Here</option>
